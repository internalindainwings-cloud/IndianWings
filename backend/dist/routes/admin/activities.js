...
router.post('/', async (req, res) => {
    try {
        const body = req.body;
        const { name, slug, location, category = 'Snow & Winter', duration = '2-4 hrs', difficulty = 'Beginner to Pro', season = 'All Year', imageUrl = '/images/gallery/gulmarg-snow.jpg', tags = [], badge = null, priceFrom = 1500, isActive = true, sortOrder = 0, } = body;
        if (!name) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const body = req.body;
        const updated = await prisma_1.prisma.activity.update({
            where: { id },
            data: {
