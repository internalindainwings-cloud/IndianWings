...
function generatePackageSlug(title: string, duration: string): string {
  const durationPart = duration.replace(/\s/g, '-').replace(/[^a-z0-9-]/gi, '').toLowerCase();
  return `${slugify(title)}-${durationPart}`;
}

function buildItineraryPayload(body: Record<string, unknown>): unknown {
  ...
  try {
    ...
    const newPackage = await prisma.package.create({
      data: {
        ...
        galleryUrls: Array.isArray(body.galleryUrls) ? body.galleryUrls as string[] : [],
        ...
        inclusions: Array.isArray(body.inclusions) ? body.inclusions as string[] : [],
        ...
        itinerary: buildItineraryPayload(body) as any,
...
router.get('/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const pkg = await prisma.package.findUnique({ where: { id }, include: { category: true } });
    if (!pkg) {
...
router.put('/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (body.tagColor !== undefined || body.cardAnimation !== undefined) updateData.tagColor = body.cardAnimation || body.tagColor || null;
    if (body.imageUrl !== undefined) updateData.imageUrl = body.imageUrl;
    if (body.videoUrl !== undefined) updateData.videoUrl = body.videoUrl && String(body.videoUrl).trim() !== '' ? String(body.videoUrl).trim() : null;
    ...
    if (body.reviewCount !== undefined) updateData.reviewCount = parseInt(String(body.reviewCount), 10);
    ...
    if (body.startingPrice !== undefined) updateData.startingPrice = parseInt(String(body.startingPrice), 10);
    if (body.originalPrice !== undefined) updateData.originalPrice = body.originalPrice ? parseInt(String(body.originalPrice), 10) : null;
    ...
    const updated = await prisma.package.update({ where: { id }, data: updateData });
