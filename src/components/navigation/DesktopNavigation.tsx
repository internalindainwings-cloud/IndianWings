...
export const DesktopNavigation = ({ isScrolled: _isScrolled = false }: { isScrolled?: boolean }) => {
  ...
  return (
    <nav className="flex items-center gap-0.5 lg:gap-1 xl:gap-1.5 ml-auto">
      <ul className="flex items-center gap-0 lg:gap-0.5 xl:gap-1 font-manrope text-[11px] lg:text-[11.5px] xl:text-[12px] font-semibold tracking-wide transition-all duration-300 rounded-full px-1.5 lg:px-2 xl:px-2 py-1 lg:py-1.5 border border-warm-white/10 bg-midnight/60 backdrop-blur-md text-warm-white">
      ...
      <div className="flex items-center pl-1 lg:pl-1 xl:pl-1.5 border-l border-warm-white/30 shrink-0">
        <button
          type="button"
          onClick={() => openModal({ source: 'desktop_nav_cta' })}
          className="inline-flex items-center gap-1 font-manrope text-[11px] lg:text-[11.5px] xl:text-[12px] font-bold text-midnight bg-[#C5A45E] rounded-full hover:bg-[#b5944e] transition-all whitespace-nowrap hover:-translate-y-0.5 shrink-0 cursor-pointer group px-2.5 py-1.5 lg:px-3 lg:py-1.5 xl:px-3 xl:py-1.5"
        >
