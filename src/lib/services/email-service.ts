...
/**
 * Sends a rich, responsive HTML Itinerary email to the traveler.
 * Gracefully falls back to simulated delivery if SMTP credentials are not yet configured.
 */
    ...
    <tr>
      <td align="center">
        ...
        <table width="600" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; width: 100%; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
          ...
          <!-- Header Banner -->
          <tr>
            <td style="background: linear-gradient(135deg, #0B1F2A 0%, #1e3a4b 100%); padding: 32px 28px; text-align: left; color: #ffffff;">
              <div style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; color: #d98f5b; margin-bottom: 6px;">
                THE INDIAN WINGS COMPANY • SRINAGAR
              </div>
              <h1 style="margin: 0 0 10px 0; font-size: 24px; font-weight: 800; color: #ffffff; line-height: 1.2;">
                ${packageTitle}
              </h1>
              <div style="font-size: 13px; color: #cbd5e1;">
                <span style="background-color: rgba(217, 143, 91, 0.25); color: #fed7aa; padding: 3px 8px; border-radius: 6px; font-weight: 600; margin-right: 8px;">
                  ${duration}
                </span>
                <span>Starting ${formattedPrice} / person</span>
              </div>
            </td>
          </tr>
          ...
          <!-- Intro Greeting -->
          <tr>
            <td style="padding: 28px 28px 16px 28px; color: #334155; font-size: 14px; line-height: 1.6;">
              <p style="margin: 0 0 12px 0;">Dear <strong>${displayName}</strong>,</p>
              <p style="margin: 0 0 16px 0;">
                Thank you for your interest in exploring Kashmir with <strong>The Indian Wings Company</strong>! Here is your requested day-by-day travel itinerary for <strong>${packageTitle}</strong>.
              </p>
              <div style="padding: 12px 16px; background-color: #fffbeb; border-radius: 8px; border: 1px solid #fde68a; font-size: 12px; color: #92400e; margin-bottom: 20px;">
                📍 <strong>Primary Route:</strong> ${routeString}
              </div>

              <!-- Day-by-Day Timeline Section -->
              <h2 style="font-size: 18px; color: #0B1F2A; margin: 24px 0 14px 0; border-bottom: 2px solid #f1f5f9; padding-bottom: 8px;">
              ...
              <div style="margin-top: 30px; text-align: center; background: #fafaf9; border-radius: 12px; padding: 24px 18px; border: 1px solid #e7e5e4;">
                <h3 style="margin: 0 0 8px 0; font-size: 16px; color: #0B1F2A;">
                  Want to customize these dates or upgrade your stay?
                </h3>
                <p style="margin: 0 0 16px 0; font-size: 12px; color: #64748b;">
                  Our local Srinagar travel team is ready to personalize this itinerary for your exact dates and group size.
                </p>
                <a href="${whatsappBookingUrl}" target="_blank" style="display: inline-block; background-color: #25D366; color: #ffffff; font-weight: 700; font-size: 14px; text-decoration: none; padding: 12px 26px; border-radius: 50px; box-shadow: 0 3px 12px rgba(37,211,102,0.3);">
                  💬 Chat With Us on WhatsApp
                </a>
              </div>

              <!-- 24x7 Srinagar Support -->
              <div style="margin-top: 24px; padding: 14px 16px; background-color: #f8fafc; border-radius: 8px; font-size: 12px; color: #64748b; text-align: center;">
                📞 <strong>24/7 Srinagar Helpline:</strong> ${siteConfig.contact.displayPhone} &nbsp;|&nbsp; ✉️ <strong>Email:</strong> ${siteConfig.contact.email}
              </div>
            </td>
          </tr>
          ...
          <!-- Footer -->
          <tr>
            <td style="padding: 20px 28px; background-color: #0B1F2A; text-align: center; color: #94a3b8; font-size: 11px;">
              <p style="margin: 0 0 4px 0; color: #cbd5e1; font-weight: 600;">
                The Indian Wings Company — Srinagar, Kashmir
              </p>
              <p style="margin: 0;">
...
`;
  ...
  // If SMTP is NOT configured, simulate cleanly without failing
  if (!host || !user || !pass) {
    console.info(
      `[EmailService] SMTP credentials not set in .env. Simulated sending itinerary email for "${packageTitle}" to ${toEmail}.`
    );
    return {
      success: true,
      simulated: true,
  ...
  try {
    ...
    console.info(`[EmailService] Itinerary successfully dispatched to ${toEmail} (MessageID: ${info.messageId})`);
    return {
      success: true,
      simulated: false,
    ...
    console.error(`[EmailService] Failed to send email to ${toEmail}:`, err?.message || err);
    return {
      success: false,
